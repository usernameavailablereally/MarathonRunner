...
namespace Core.Controllers
{
    public class GameInputController : ITickable
    {
        ...
        private void CheckForMouseInput()
        {
            ...
            if (IsLeftButtonDown())
            {
                if (TryGetItemUnderCursor(out IClickable itemController)  && itemController != null)
                {
                    _dispatcherService.Dispatch(new ClickableClickedEvent(itemController));
                }
                _isWaitingForMouseUp = true;
            }
        }
        
        private bool TryGetItemUnderCursor(out IClickable itemComponent)
        {
            itemComponent = null;
            ...
            itemComponent = hit.collider.GetComponent<IClickable>();
